...
export const colorPlugin = plugin(({addUtilities}) => {
  addUtilities({
    '.color-scheme-light': colorSchemeLight_,
    '.color-scheme-dark': colorSchemeDark_,
    '.color-scheme-auto': {
      ...colorSchemeLight_,
      '@media (prefers-color-scheme: dark)': colorSchemeDark_,
    },
  });
});
